...
use crate::{FrostParticipantInternalData, RustFfiError};
...
impl TryFrom<(i32, FrostParticipantInternalData)> for RustTypeActivityInfo {
    type Error = RustFfiError;

    fn try_from(values: (i32, FrostParticipantInternalData)) -> Result<Self, Self::Error> {
...
impl RustTypeActivityState {
    pub fn stringify(&self) -> &'static str {
        match self {
            ...
            Self::Aggregate => "Threshold Signing Finalized",
        }
    }
}
...
impl From<ActivityState> for RustTypeActivityState {
    fn from(state: ActivityState) -> Self {
        match state {
            ...
            ActivityState::Aggregate => Self::Aggregate,
        }
    }
}
...
impl From<RustTypeActivityState> for ActivityState {
    fn from(state: RustTypeActivityState) -> Self {
        match state {
            ...
            RustTypeActivityState::Aggregate => Self::Aggregate,
        }
    }
}
